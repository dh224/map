  ...
  methods: {
        ...
        if (startX == endX && startY == endY) {
          ...
          if (Math.abs(p) == Number.POSITIVE_INFINITY) {
            ...
            if (endX < startX) {
              ...
              newY = this.peopleGra[i].geometry.latitude - (p / Math.sqrt(1 + p * p)) * v * 0.7;
            } else {
              newX =
                this.peopleGra[i].geometry.longitude + (1 / Math.sqrt(1 + p * p)) * v;
              newY = this.peopleGra[i].geometry.latitude + (p / Math.sqrt(1 + p * p)) * v* 0.7;
    ...
    getOnePath(type,pointnumbers){
      if(type == "single"){
        ...
        //C: 退休老年人 从家中出发去公园或者娱乐场所，随后再次出发前往另一个公园、娱乐场所。每个场所停留的时间较短，不太可能在去吃饭。有可能会前往医院。交通方式一般是步行。
        //25%戴口罩,25%接种疫苗,年龄在 60~80之间。
        //D: 乱走之人 从家中出发，在娱乐场所和吃饭场所之间交流，不一定回家。距离较远可能会选择驾车。
        //几乎不带口罩,50%接种疫苗,年龄在18 ~ 40之间.
      }
    },
    async getPathWithParttern(parttern){
      if(parttern == 0){
        var randomHome = this.HousePositions[Math.floor(Math.random() * this.HousePositions.length)];
        var randomWorkplace = this.workPositions[Math.floor(Math.random() * this.workPositions.length)]
        var tempPath = await sendRequest(randomHome,randomWorkplace)
        while(Math.floor(Math.random() * 10) >= 5){
          if(Math.floor(Math.random() * 10) >= 8){
            var randomEnt = this.entertainmentPositions[Math.floor(Math.random() * this.entertainmentPositions.length)];
            var tempPath2 = await sendRequest(randomWorkplace,randomEnt)
          }else{
            var randomEat =this.EatingPositions[Math.floor(Math.random() * this.EatingPositions.length)]; 
            var tempPath2 = await sendRequest(randomWorkplace,randomEat)
          }
